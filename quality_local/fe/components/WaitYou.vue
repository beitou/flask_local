    ...
    <a-card title="基础事件">
      <a-form :form="form">
        ...
        <a-form-item
          ...
          label="保险师"
        >
          <a-input v-model="event.staff_id" />
        </a-form-item>
        <a-form-item
          :label-col="{ span: 5 }"
          :wrapper-col="{ span: 17 }"
          label="用户"
        >
          <a-input v-model="event.uuid" placeholder="请填写用户UUID" />
        </a-form-item>
        <a-form-item
          :label-col="{ span: 5 }"
          :wrapper-col="{ span: 17 }"
...
export default {
  data() {
    return {
      ...
      events: [
        {
          value: 'admin_user_external_friend_event',
          label: '企业微信添加好友事件',
        },
        {
  ...
  methods: {
    ...
    handleTimeChangge(value) {
      this.event.event_time = Math.round(value.valueOf() / 1000)
      this.event.created_at = Math.round(value.valueOf() / 1000)
    },
